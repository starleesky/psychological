...
public class InfomationController {
	...
	public void save(InfomationDto infomation, HttpServletResponse response, HttpSession httpSession) {
		...
		if (infomation.getValidTime() == null || "".equals(infomation.getValidTime())) {
	...
	public String moreInfo(PageDto pageDto, Infomation infomation, HttpSession session, Pager<InfomationDto> pager) {
		...
		for (InfomationDto info : pager.getItems()) {
			data.append("<li class=\"pro-box\">")
			    ...
			    .append("<img src=\"").append(imgHost).append(info.getImgUrl()).append("/small")
			    .append("\" class=\"jImg\" data-url=\"\" />")
			    .append("</a>")
			...
			data.append("<p class=\"col-6\"> 截止日期:<span>")
			    .append(info.getEndTime() == null ? "" : sdf.format(info.getEndTime())).append("</span> </p>")
			    .append("</div>");
