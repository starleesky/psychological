define(
		...
		function($, url, ajax, box, Lazyload, LazyStream) {
			...
			oBigGoodsCatagory.change(function() {
				$("#catagoryBigId").val(
						$('#infoSearchForm').find('select[name=catagoryBig]')
								.val());
				$("#catagoryBigName").val(
						$('#infoSearchForm').find('select[name=catagoryBig]')
			...
			var getMiddle = function() {
				var n = oBigGoodsCatagory.val() == '' ? '-9999999'
						: oBigGoodsCatagory.val();
				$.getJSON(url.listGoodsCatagory, {
					id : n
				}, function(data) {
			...
			oMiddleGoodsCatagory.change(function() {
				$("#catagoryMidId").val(
						$('#infoSearchForm').find('select[name=catagoryMid]')
								.val());
				$("#catagoryMidName").val(
						$('#infoSearchForm').find('select[name=catagoryMid]')
								.find("option:selected").text());
			...
			var getSmall = function() {
				var n = oMiddleGoodsCatagory.val() == '' ? '-9999999'
						: oMiddleGoodsCatagory.val();
				$.getJSON(url.listGoodsCatagory, {
					id : n
				}, function(data) {
			...
			oSmallGoodsCatagory.change(function() {
				$("#catagoryId").val(
						$('#infoSearchForm').find('select[name=catagorySmall]')
								.val());
				$("#catagoryName").val(
						$('#infoSearchForm').find('select[name=catagorySmall]')
								.find("option:selected").text());
			...
			var getBrand = function() {
				var n = oSmallGoodsCatagory.val() == '' ? '-9999999'
						: oSmallGoodsCatagory.val();
			...
			oBrand.change(function() {
				$("#brandId").val(
						$('#infoSearchForm').find('select[name=brand]').val());
				$("#brandName").val(
						$('#infoSearchForm').find('select[name=brand]').find(
			...
			oModels
					.change(function() {
						$("#modelId").val(
								$('#infoSearchForm')
										.find('select[name=models]').val());
						$("#modelName").val(
								$('#infoSearchForm')
										.find('select[name=models]').find(
												"option:selected").text());
					});

			var initOtherSel = function() {
				$("select[name='sellTypeSel']").val($("#sellType").val());
				$("select[name='equipmentConditionSel']").val(
						$("#equipmentCondition").val());
