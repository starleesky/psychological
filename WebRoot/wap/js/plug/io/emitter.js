...
var guid = function() { return new Date() * Math.random() & 0xFFFF }();

var hid = '_eid\u03A6' + guid.toString(36)

// hook method for object extra data getter & setter
var hookCache = {}
function hook(o, create) {
  var id;
  return create === null
    ? o[hid] && ( delete hookCache[o[hid]], delete o[hid] )
    : (
        id = ( o[hid] || create && ( id = ++guid, hookCache[id] = {}, o[hid] = id ) ),
        id && hookCache[id]
      )
}
...
__proto__.addListener = function(events, callback, context, single) {
  ...
  cache = hook(this, 1) // init hook store if not exists
...
__proto__.removeListener = function(events, callback, context) {
  ...
  if (!(cache = hook(this))) return this
  if (!(events || callback || context)) {
    forIn(cache, function(v, k) { delete cache[k] })
    hook(this, null) // release hook store
    return this
  }
  ...
  while (event = events.shift()) {
    ...
    for (i = list.length; --i >= 0; ) {
      ...
      if (!(callback && (handle !== callback && handle.guid !== callback.guid) ||
...
function triggerEventEmitter(type, list, args, context) {
  var pass = true
  if (list) {
    var i = -1, opt, fn, ctx, eventObj= {type: type, timeStamp: +new Date}
    while (opt = list[++i]) {
