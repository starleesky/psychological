...
public class AdminInfomationController {
	...
	public Result<String> update(@RequestBody InfomationDto infomation, HttpSession httpSession) {
		...
		if (AuditRecordEnum.audit_status_success.code().equals(infomation.getAuditStatus())) {
			notice.setContent(TsjxConstant.information_audit_success
					.replace("%1", infomation.getBrandName() + "/" + infomation.getModelName()).replace("%2", remark));
		} else {
			notice.setContent(TsjxConstant.information_audit_failure
					.replace("%1", infomation.getBrandName() + "/" + infomation.getModelName()).replace("%2", remark));
		}
	...
	@ResponseBody
	public Result<?> saveInsCityRate(@RequestParam("file") MultipartFile file, Integer companyCityId,
