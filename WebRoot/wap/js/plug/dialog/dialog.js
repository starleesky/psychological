...
define(function(require) {
    ...
    artDialog.create = function(options) {
        ...
        $popup.on('click', '[data-id]', function(event) {
            ...
            if (!$this.attr('disabled')) {// IE BUG
                return that._trigger($this.data('id'));
            }
        });
    ...
    $.extend(prototype, {
        ...
        content : function(html) {
            ...
            var _html = this._getTypeHtml(html);
            _html && (html = _html);
         ...
         *            弹窗类型
         *
         * @author djune update@2014.7.22
         */
        type : function(type) {
            this.options.type = type;
            var eCntHtml = this._$('content').find('.ui-dialog-cnt-html');
            if (eCntHtml && eCntHtml.length == 0) {
                eCntHtml = this._$('content');
            }
            return this.content(eCntHtml.html());
        },
         ...
         */
        _getTypeHtml : function(html) {
            var type = this.options.type, strHtml = '', strIcon = '';
            ...
            if (strIcon.length > 0) {
                strHtml = '<div class="ui-dialog-cnt-icon icon iconfont">' + strIcon + '</div><div class="ui-dialog-cnt-html">' + html + '</div>';
                var strCSS = 'ui-dialog-type ui-dialog-tips ui-dialog-warn ui-dialog-question ui-dialog-error ui-dialog-ok ui-dialog-loading ui-dialog-null';
                this._$('dialog').removeClass(strCSS);
                this._$('dialog').addClass('ui-dialog-type ui-dialog-' + type);
                return strHtml;
            }
