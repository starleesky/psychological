    ...
    angular.module('App')
        ...
        .controller('mainCtrl', ['$scope', '$http', 'commonList', '$modal', 'address', 'Upload',
            function ($scope, $http, commonList, $modal, address, $upload) {
                ...
                list.filter.key = '';
                list.url = "/admin/models/list/getData";
                ...
                $scope.edit = function (rw, $index) {
                    ...
                    index = $index;
                    modal = $modal.open({
                        templateUrl: angular.path + '/resources/templates/models/models_edit.html?data=' + new Date(),
                        backdrop: 'static',
                        scope: $scope,
                        size: 'lg',
                        resolve: {
                ...
                $scope.deleteOne = function (data, $index) {
                    if (confirm("确认删除？")) {
                        $http.get(angular.path + "/admin/models/del?id=" + data.id)
                ...
                $scope.updateParam = function () {
                    ...
                    //alert($scope.user.qq);
                    //return;
                    $http.post(angular.path + "/admin/models/update", $scope.user)
                        .success(function (resp) {
                            if (resp.result) {
                ...
                $scope.add = function () {
                    $scope.user = {};
                    modal = $modal.open({
                        templateUrl: angular.path + '/resources/templates/models/models_add.html?data=' + new Date(),
                        backdrop: 'static',
                        scope: $scope,
                        size: 'lg',
                        resolve: {
                ...
                $scope.addParam = function () {
                    $http.post(angular.path + "/admin/models/add", $scope.user)
                        .success(function (resp) {
                            if (resp.result) {
                                list.fetch();
                                modal.close();
